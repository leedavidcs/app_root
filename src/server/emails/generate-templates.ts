...
const toDefaultExport = (text: string) => `export default \`${text}\`
`;
...
export const generateTemplates = (options: IGenerateFromMjmlOptions) => {
	...
	Object.keys(emailHtmlDict).forEach((key) => {
		const outputPath: string = path.join(output, `${key}.ts`);
		const emailHtml: string = emailHtmlDict[key];

		fs.ensureFileSync(outputPath);
		fs.writeFileSync(outputPath, toDefaultExport(emailHtml), {
			encoding: "utf8",
			flag: "w"
		});
	});
